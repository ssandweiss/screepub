...
- **A script can keep its own formatting.** `screepub settings` saves the way
  you want one script laid out, in a small file beside it. Every later
  conversion of that script uses those settings and says so while it runs. A
  script you have never tuned converts exactly as it did before.
